...
const CafeCard = ({ item }) => {
  const dayInfo = getDayInfo(item.workingDays);
  const {name, ownerName, bio, city, state, image, ownerImage, _id} = item;
  ...
  return (
    <div className="max-w-[370px] md:h-[535px] m-4 ssm:h-[555px] rounded-lg overflow-hidden shadow-lg bg-white">
      ...
      <div className="px-6 py-2">
         <Link to={`/cafes/${_id}`} onClick={() => actions({type:'SET_SELECED_CAFE', payload:item})} className={`${linkBtnStyle} `}>
            View Cafe
          </Link>
      </div>
    </div>
