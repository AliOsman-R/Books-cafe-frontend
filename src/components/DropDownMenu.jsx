...
const DropDownMenu = () => {
    ...
    const { user, isAuth, actions } = useContext(Context);
        ...
        <div className="">
            <div className="flex cursor-pointer" ref={profileRef} onClick={handleClick}>
                ...
                <div className='hover:text-primaryColor cursor-pointer flex justify-center'>
                    <img className='size-[35px] rounded-full' src={user?.profileImage}/> 
                    {openedMenu?<MdArrowDropDown size={20}/> : <MdArrowDropUp size={20}/>}
                </div>
                    :
                <div className='hover:text-primaryColor cursor-pointer flex justify-center'>
                    <CgProfile className='size-[35px] rounded-full'/>
                    {/* <MdArrowDropDown className='size-[23px]'/> */}
                    {openedMenu?<MdArrowDropDown size={20}/> : <MdArrowDropUp size={20}/>}
                </div>
                }   
            </div>
            <div ref={menuRef} className={`absolute top-full right-[65px]  mt-2 min-w-[250px] shadow-lg  max-h-0 z-[1000] overflow-hidden transition-max-height duration-500 ${openedMenu ? 'max-h-[280px]' : ''}`}>
                    ...
                    <div className="flex flex-col">
                        {sidebarList.map(item => (
                            <div className="flex flex-col" key={uuidv4()}>
                                <Link to={`/user/profile/${item.path}`} className={`${linkStyle}`}>
