...
useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!validatePassword(credentials.password)) {
        setValidationError('Password must include at least one uppercase letter, one lowercase letter, one number, one special character, and be at least 8 characters long.');
      return;
    }

    setBtnLoading(true)
    httpRequest.post('/auth/signup', credentials)
      .then(({data}) => {
        console.log(data);
        navigate('/auth/login')
      ...
      <AuthCard title={'Sign Up'} text={'Welcome to CafeX'}>
         ...
         <div>
          <form onSubmit={handleSubmit} className="space-y-3 ssm:p-3 lg:p-8">
            ...
            <Container labelName="Full Name">
                <PrimaryInput required value={credentials.name} onChange={handleChange} name="name" type="text" placeholder="Your full name" />
            </Container>
            <Container labelName="Email">
                <PrimaryInput required value={credentials.email} onChange={handleChange} name="email" type="email" placeholder="Your email" />
            </Container>
            <Container labelName="Password">
                <PrimaryInput required value={credentials.password} onChange={handleChange} name="password" type="password" placeholder="Your password" />
            </Container>
            <PrimaryButton className='w-full' disabled={btnLoading} type="submit">
        ...
        <div className="text-center mt-4">
          <p className="text-gray-600">Already have an account? 
            <Link to="/auth/login" className={linkStyle}> Log in</Link>
          </p>
        </div>
    </AuthCard> 
  );
